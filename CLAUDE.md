...
Read `docs/architecture.md` before making any non-trivial change. It's a decision log (51 numbered decisions) covering *why* each major choice was made and what alternatives were rejected — routing logic, schema design, datastore choice, model selection, etc. Don't reverse or contradict a numbered decision without flagging it; if new work touches one, reference the decision number in commit messages/comments the way existing code does (e.g. `# decisions #9, #11`). Note decision #49 already specifies the critic-synthesizer's design (forced tool-use on `synthesize_answer`, Sonnet, always runs even on empty evidence, citations as evidence-list indices) — it's a settled spec to implement against, not something to redesign from scratch when that work starts.

**Current implementation state** (see README.md progress checklist, though it lags this file — trust this file and the code over it): ingestion is fully built end-to-end — job lifecycle, a Postgres-native job queue, the standalone worker that executes clone/parse/embed/index, retrieval indexing, and code-navigation indexing. The full agentic query pipeline is now live for single-shot queries: the **planner** (`agent_orchestrator/planner.py`, decision #42), **code-navigation-as-agent** (`agent_orchestrator/code_navigation_agent.py`, decisions #43, #44), **retriever-as-agent** (`agent_orchestrator/retriever_agent.py`, decision #45), **critic-synthesizer** (`agent_orchestrator/critic.py`, decision #49), and the **orchestrating function** that wires them together (`agent_orchestrator/query.py::answer_query`, decision #50) are all implemented — `POST /codebases/{id}/query` calls the real pipeline and returns a real `answer`/`refused` response instead of `501`. `POST /conversations/{id}/messages` still intentionally returns `501`, because LangGraph conversation orchestration (decisions #29, #31) is a separate, not-yet-built milestone — `answer_query`'s `prior_evidence` parameter exists for that future caller but is always `[]` on the current single-shot path (decision #41). Don't route conversation messages through `answer_query` "to unblock them faster" — decision #31 keeps single-shot and conversation flows as deliberately separate entry points.
...
- **`packages/rag_core`** — ingestion and retrieval engine, no FastAPI/HTTP dependency:
  - `parsing/` — tree-sitter based structural parsing (`tree_sitter_python` grammar, decision #48 — chosen over the `ast` module specifically because `ast` strips comments before parsing and can't support tree-based comment attachment), Python only for v1, shared by both retrieval chunking and code-navigation indexing rather than built twice. Attaches docstrings/comments to their owning function/class node via tree structure, not proximity heuristics.
...
- **`packages/agent_orchestrator`** — planner, retriever-as-agent, code-navigation-as-agent, critic-synthesizer, and the orchestrating function that routes between them. Depends on `shared` + `rag_core`, never on `services/api` (dependency direction is services → packages, never the reverse — `query.py` defines its own `QueryResult`/`QueryCitation` dataclasses rather than importing `services/api`'s pydantic models). Implemented: the **planner** (`planner.py` — a forced-tool-use Claude call producing a `PlannerDecision`, decisions #33, #34, #36, #41, #42), **code-navigation-as-agent** (`code_navigation_agent.py` — a plain function with no LLM/API dependency that calls both `find_definition` and `find_references` for every planner-selected symbol name and maps results onto `EvidenceItem`, decisions #43, #44), **retriever-as-agent** (`retriever_agent.py` — wraps `rag_core`'s `hybrid_search` with decision #32's two-stage mechanism: a top-1-rerank-score confidence gate (`RERANK_CONFIDENCE_THRESHOLD`, env-configurable, default `0.5`), and if it fails, exactly one forced-tool-use Claude reformulation call (mirroring the planner's `PlannerClient`/`AnthropicPlannerClient` shape, including a `ReformulationValidationError` guard against missing fields or `max_tokens` truncation, decision #51) before returning whatever the retry produces, unconditionally — decision #45), the **critic-synthesizer** (`critic.py` — a forced-tool-use Sonnet call (decision #36) producing a `CriticDecision` from a question plus a *numbered* evidence list; same `PlannerClient`-shaped pattern (`CriticClient` protocol, `AnthropicCriticClient`, plain-Python `_validate_decision` raising `CriticValidationError`); refuses on empty evidence rather than answering from pretraining, and every `cited_evidence_indices` entry is validated in-range against the evidence list actually sent — decisions #11, #12, #16, #26, #35, #49; a required field can come back missing from the model's tool_use response even under forced tool_choice (observed live, independent of prompt wording), so `synthesize` checks for missing fields and `stop_reason == "max_tokens"` before indexing `tool_input`, and retries the same call exactly once with the failure fed back before raising for real — decision #51, and the planner does the same for its own call), and the **orchestrating function** (`query.py::answer_query` — decision #50: calls the planner, branches on `agents_needed` to invoke whichever agents were requested sequentially (not in parallel — deferred to v2), concatenates their evidence, calls the critic over the combined list, then maps `cited_evidence_indices` back into `QueryCitation`s by indexing into that same list; `PlannerValidationError`/`CriticValidationError` propagate uncaught rather than being disguised as a refusal, surfacing as a 500). Each agent has a `Fake*Client` fixture in `tests/conftest.py` (`fake_planner_client`, `fake_reformulation_client`, `fake_critic_client`), reused by `test_query.py`'s pure unit tests of `answer_query`'s branching logic (stubbed evidence-gathering functions, no real Postgres/Qdrant needed).
- **`services/api`** — FastAPI HTTP layer: routers per concern (`jobs`, `codebases`, `conversations`), Pydantic request/response models in `api/models.py`, shared dependency wiring (Postgres connection, Qdrant client, Voyage/planner/reformulation/critic clients, all lazily-constructed module-level singletons) in `api/deps.py`. `codebases.py`'s `query_codebase` handler validates the codebase exists (404) and is `ready` (409), then calls `agent_orchestrator.query.answer_query` and does the trivial field-by-field mapping from its `QueryResult`/`QueryCitation` dataclasses onto `QueryResponse`/`Citation`. Schema migrations run at app startup via `lifespan` in `api/main.py` — **order matters**: jobs schema must run before conversations schema (FK dependency), see the comment in `main.py`.
...
- **Critic never re-triggers retrieval.** Retry/reformulation logic belongs to the retriever and code-navigation tool during the collection phase (decision #12, #32); the critic-synthesizer only evaluates a closed evidence set and either answers or refuses — it's a gate, not an orchestrator. It does retry its own call once on a malformed response (decision #51), but that retry re-runs synthesis over the same fixed evidence, not new evidence gathering.
