...
- **`packages/agent_orchestrator`** — planner, retriever-as-agent, code-navigation-as-agent, critic-synthesizer, and the routing between them. Depends on `shared` + `rag_core`. Only the **planner** is implemented (`planner.py` — a forced-tool-use Claude call producing a `PlannerDecision`, decisions #33, #34, #36, #41, #42); retriever-as-agent and critic-synthesizer are not yet built, which is why the query endpoints still return `501`.
