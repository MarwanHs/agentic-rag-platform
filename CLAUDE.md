...
**Current implementation state** (see README.md progress checklist, though it lags this file — trust this file and the code over it): ingestion is fully built end-to-end — job lifecycle, a Postgres-native job queue, the standalone worker that executes clone/parse/embed/index, retrieval indexing, and code-navigation indexing. Of the agentic query pipeline, the **planner** (`agent_orchestrator/planner.py`, a forced-tool-use Claude call, decision #42), **code-navigation-as-agent** (`agent_orchestrator/code_navigation_agent.py`, decisions #43, #44), **retriever-as-agent** (`agent_orchestrator/retriever_agent.py`, decision #45), and the **critic-synthesizer** (`agent_orchestrator/critic.py`, a forced-tool-use Sonnet call, decision #49) are all implemented and independently tested; the function that orchestrates between them — combining the two agents' evidence, calling the critic, and mapping its output onto the API response shape (`cited_evidence_indices` → `Citation` objects, `agents_needed` → `sources_used`) — is not, so `POST /codebases/{id}/query` and `POST /conversations/{id}/messages` still intentionally return `501` rather than fabricate a response. LangGraph conversation orchestration is also not implemented yet. Don't "fix" these 501s by stubbing a fake answer/refusal.
...
- **`packages/agent_orchestrator`** — planner, retriever-as-agent, code-navigation-as-agent, critic-synthesizer, and the routing between them. Depends on `shared` + `rag_core`, never on `services/api` (dependency direction is services → packages, never the reverse). Implemented so far: the **planner** (`planner.py` — a forced-tool-use Claude call producing a `PlannerDecision`, decisions #33, #34, #36, #41, #42), **code-navigation-as-agent** (`code_navigation_agent.py` — a plain function with no LLM/API dependency that calls both `find_definition` and `find_references` for every planner-selected symbol name and maps results onto `EvidenceItem`, decisions #43, #44), **retriever-as-agent** (`retriever_agent.py` — wraps `rag_core`'s `hybrid_search` with decision #32's two-stage mechanism: a top-1-rerank-score confidence gate (`RERANK_CONFIDENCE_THRESHOLD`, env-configurable, default `0.5`), and if it fails, exactly one forced-tool-use Claude reformulation call (mirroring the planner's `PlannerClient`/`AnthropicPlannerClient` shape) before returning whatever the retry produces, unconditionally — decision #45), and the **critic-synthesizer** (`critic.py` — a forced-tool-use Sonnet call (decision #36) producing a `CriticDecision` from a question plus a *numbered* evidence list; same `PlannerClient`-shaped pattern (`CriticClient` protocol, `AnthropicCriticClient`, plain-Python `_validate_decision` raising `CriticValidationError`, no retry on failure); refuses on empty evidence rather than answering from pretraining, and every `cited_evidence_indices` entry is validated in-range against the evidence list actually sent — decisions #11, #12, #16, #26, #35, #49). Each agent has a `Fake*Client` fixture in `tests/conftest.py` (`fake_planner_client`, `fake_reformulation_client`, `fake_critic_client`) for use by the orchestrating function's tests. That orchestrating function itself — combining evidence, calling the critic, mapping `cited_evidence_indices` → `Citation` objects and `agents_needed` → `sources_used` — is not yet built, which is why the query endpoints still return `501`.
